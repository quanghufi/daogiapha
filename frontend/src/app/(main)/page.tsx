...
function LotusCorner({
  ...
  return (
    <div className={`absolute ${posClass}`}>
      <div className="relative w-[84px] md:w-[116px] aspect-[296/205] overflow-hidden opacity-80">
        <Image
          src="/tree-assets/lotus-decoration-trim.png"
          alt="Hoa sen trang trí"
          fill
          sizes="116px"
          className={`object-cover ${align === 'left' ? 'object-left' : 'object-right'} ${mirrorTop ? '-scale-y-100' : ''}`}
        />
      </div>
    </div>
  );
        ...
        <div className="container relative z-10 mx-auto px-4 py-10 md:py-14">
          <div className="relative mx-auto max-w-5xl rounded-2xl border-2 border-amber-700/70 bg-amber-50/80 px-5 py-8 shadow-[0_16px_36px_rgba(125,85,15,0.18)] backdrop-blur-sm md:px-10 md:py-10">
            <div className="pointer-events-none absolute inset-0 z-0">
              <LotusCorner position="tl" mirrorTop align="left" />
              <LotusCorner position="tr" mirrorTop align="right" />
              <LotusCorner position="bl" align="left" />
              <LotusCorner position="br" align="right" />
            </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {features.map((feature) => (
            <Card key={feature.title} className="group transition-shadow hover:shadow-lg">
              ...
              <CardContent>
                <Button variant="ghost" asChild className="transition-transform group-hover:translate-x-1">
